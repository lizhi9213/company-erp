            ...
            if (!!conf) {
                try {
                    ...
                    oSipStack = new SIPml.Stack({
                        ...
                        ice_servers: [{ url: 'stun:66.228.45.110:3478' }]
                    })
        ...
        function callfunback(resp) {
            ...
            if (resp.code == '200') {
                if (_outPhone != '' || _inPhone != '') {
                    if (_outPhone != '') {
                        data.callcode = resp.data[0].callcode;
                        data.phone = _outPhone;
                        data.type = 1;
                        if (_userstatus != resp.data[0].userstatus) {
                            _userstatus = resp.data[0].userstatus;
                            regfunback();
                        }
                    } else if (_inPhone != '') {
                        data.callcode = resp.data[0].callcode;
    ...
    function getHttp(url, callback, data) {
        ...
        request.onreadystatechange = function () {
            ...
            if (request.status === 200) {
                if (callback) {
                    respjson = eval('(' + request.response + ')');
                    callback(respjson);
                }
            } else {
                callback();
