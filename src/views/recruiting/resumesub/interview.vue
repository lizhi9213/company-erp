  ...
  <div>
      ...
      <div slot="button">
        <el-dropdown @command="buttonCommand">
          ...
          <el-dropdown-menu slot="dropdown">
            <el-dropdown-item :command="{key:'interview'}">面试通过</el-dropdown-item>
            <el-dropdown-item :command="{key:'review'}">安排复试</el-dropdown-item>
    ...
    <el-dialog :visible.sync="viewModal" title="标记结果" width="600px">
      <el-form :model="viewForm" ref="viewForm" label-width="110px">
        ...
        <el-form-item label="复试时间" prop="NextTime" :rules="{required:viewForm.status[0] === '3e85e76b-8ee8-11e8-86f8-44a84203f5fd',message:'请选择复试时间'}">
          <el-date-picker v-model="viewForm.NextTime" type="datetime" placeholder="请选择复试时间" style="width:100%" :picker-options="pickerOptions" :disabled="viewForm.status[0] !== '3e85e76b-8ee8-11e8-86f8-44a84203f5fd'" value-format="yyyy-MM-dd HH:mm:ss"></el-date-picker>
        </el-form-item>
        <el-form-item label="改约面试时间" prop="VisitTime" :rules="{required:viewForm.status[0] === 'bc462eb1-8ee5-11e8-86f8-44a84203f5fd',message:'请选择面试时间'}">
          <el-date-picker v-model="viewForm.VisitTime" type="datetime" placeholder="请选择改约面试时间" style="width:100%" :picker-options="pickerOptions" :disabled="viewForm.status[0] !== 'bc462eb1-8ee5-11e8-86f8-44a84203f5fd'" value-format="yyyy-MM-dd HH:mm:ss"></el-date-picker>
        </el-form-item>
      </el-form>
      <div slot="footer">
        <el-button @click="viewModal=false">取消</el-button>
        <el-button type="primary" :loading="saveing" @click="interview">确定</el-button>
      </div>
    </el-dialog>
...
import { getInterviewList, saveInterview } from '@/api/recruiting/interview'
...
export default {
    ...
    return {
      columns: [
        ...
        },
        {
          ...
          minWidth: 100,
          render: function(h, params) {
            const o = params.row.DealOrder + 1
            let t = ''
            switch (o) {
              case 1:
                t = '一面'
  ...
  methods: {
    ...
    buttonCommand(obj) {
      switch (obj.key) {
        case 'interview':
          ...
          break
        case 'review':
          this.openViewModel(this.selected, 2)
          break
        case 'mark':
    ...
    handleCommand(val) {
      switch (val.key) {
        case 'interview':
          ...
          break
        case 'review':
          this.openViewModel([val.ResumeId], 2)
          break
        case 'mark':
    ...
    openViewModel(id, type) {
      ...
      this.viewModal = true
      if (!type) {
        this.viewForm.status = ['73052bdd-8ee5-11e8-86f8-44a84203f5fd']
      } else if (type === 1) {
        this.viewForm.status = [
          '84cf0d61-8ee5-11e8-86f8-44a84203f5fd',
          '8700f6b0-8ee5-11e8-86f8-44a84203f5fd'
        ]
      } else if (type === 2) {
        this.viewForm.status = ['3e85e76b-8ee8-11e8-86f8-44a84203f5fd']
      }
  ...
  created() {
    ...
    this.getData()
    getDataDictTree('58742a23-8ee5-11e8-86f8-44a84203f5fd').then(res => {
