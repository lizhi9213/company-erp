  ...
  <div>
      ...
      <div slot="button">
        <el-dropdown @command="buttonCommand">
          ...
          <el-dropdown-menu slot="dropdown">
            <el-dropdown-item :command="{key:'train'}">入职</el-dropdown-item>
    ...
    <el-dialog :visible.sync="viewModal" title="标记结果" width="600px">
      ...
      <div slot="footer">
        ...
        <el-button type="primary" :loading="saveing" @click="train">确定</el-button>
      </div>
    </el-dialog>
    ...
    <el-dialog title="生成业务系统工号" :visible.sync="dialogFormVisible" width="600px">
      <el-alert type="warning" title="注意" description="选择目的系统生成业务系统工号，该操作不可逆。默认选择招聘计划所属办事处，您也可以自行修改。"></el-alert>
      <br>
      <el-cascader :options="officeOptions" change-on-select v-model="accountForm.officeId" style="width:100%"></el-cascader>
      <div slot="footer" class="dialog-footer">
        <el-button @click="dialogFormVisible = false">取 消</el-button>
        <el-button type="primary" @click="generateAccount" :loading="saveing">确 定</el-button>
      </div>
    </el-dialog>
  </div>
</template>
...
import { getTrainList, saveTrain } from '@/api/recruiting/train.js'
import { saveDealLink, addDial } from '@/api/recruiting/link'
import { generateAccount } from '@/api/crmapi/crmcall'
...
export default {
  ...
  methods: {
    ...
    handleCommand(val) {
      switch (val.key) {
        ...
        case 'link':
          ...
          break
        case 'account':
          this.openDialog(val.id, val.officeId)
          break
        default:
    ...
    openViewModel(id, type) {
      ...
      this.viewModal = true
      if (!type) {
        this.viewForm.status = ['7823ed09-8f16-11e8-86f8-44a84203f5fd']
      } else if (type === 1) {
        this.viewForm.status = ['7b38760f-8f16-11e8-86f8-44a84203f5fd']
      }
    ...
    openDialog(ResumeId, officeId) {
      Vue.set(this.accountForm, 'officeId', [officeId])
      Vue.set(this.accountForm, 'ResumeId', [ResumeId])
      this.dialogFormVisible = true
    },
    generateAccount() {
      this.saveing = true
      if (
        this.accountForm['officeId'].length > 0 &&
        this.accountForm['officeId'][0] !== undefined
      ) {
        generateAccount(this.accountForm)
          .then(res => {
            this.saveing = false
            if (res.data.code === 200) {
              this.dialogFormVisible = false
              this.$alert(
                '创建成功，工号：' + res.data.Account + '，密码：123456',
                '提示'
              )
            } else {
              this.$message({
  ...
  created() {
    ...
    this.getData()
    getDataDictTree('f7418f16-8f15-11e8-86f8-44a84203f5fd').then(res => {
