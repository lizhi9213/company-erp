<template>
  <div class="app-container">
    <el-form ref="searchform" :model="searchform" label-width="100px" class="search">
      <el-row>
        <el-col :xs="7" :sm="7" :md="7" :lg="7" :xl="7">
          <el-form-item label="员工信息" prop="name">
            <el-input v-model="searchform.name" placeholder="姓名、手机号、工号、邮箱、开户行"></el-input>
          </el-form-item>
          <el-form-item label="部门" prop="depart">
            <el-cascader :options="departOptions" v-model="depart" placeholder="请选择部门" change-on-select :clearable='true' expand-trigger="hover" @change="departChange">
            </el-cascader>
          </el-form-item>
          <el-form-item label="入职日期" prop="entryDate">
            <el-date-picker type="date" value-format="yyyy-MM-dd" placeholder="选择入职日期" v-model="searchform.entryDate" style="width: 100%;"></el-date-picker>
          </el-form-item>
        </el-col>
        <el-col :xs="7" :sm="7" :md="7" :lg="7" :xl="7">
          <el-form-item label="员工状态" prop="userStatus">
            <el-select v-model="searchform.userStatus" :clearable='true' placeholder="请选择员工状态">
              <el-option v-for="item in ustatusOption" :key="item.value" :label="item.label" :value="item.value">
              </el-option>
            </el-select>
          </el-form-item>
          <el-form-item label="职务" prop="post">
            <el-cascader :options="dutiesOption" v-model="post" placeholder="请选择职务" change-on-select :clearable='true' expand-trigger="hover" @change="jobChange">
            </el-cascader>
          </el-form-item>
          <el-form-item label="转正日期" prop="workDate">
            <el-date-picker type="date" value-format="yyyy-MM-dd" placeholder="选择转正日期" v-model="searchform.workDate" style="width: 100%;"></el-date-picker>
          </el-form-item>
        </el-col>
        <el-col :xs="7" :sm="7" :md="7" :lg="7" :xl="7">
          <el-form-item label="账号状态" prop="accountStatus">
            <el-select v-model="searchform.accountStatus" :clearable='true' placeholder="请选择账号状态">
              <el-option v-for="item in astatusOption" :key="item.value" :label="item.label" :value="item.value">
              </el-option>
            </el-select>
          </el-form-item>
          <el-form-item label="招聘来源" prop="userSource">
            <el-select v-model="searchform.userSource" :clearable='true' placeholder="请选择招聘来源">
              <el-option v-for="item in recruitOption" :key="item.value" :label="item.label" :value="item.value">
              </el-option>
            </el-select>
          </el-form-item>
          <el-form-item label="出生月份" prop="birdate">
            <el-date-picker type="month" value-format="yyyy-MM" placeholder="选择出生月份" v-model="searchform.birdate" style="width: 100%;"></el-date-picker>
          </el-form-item>
        </el-col>
      </el-row>
      <el-form-item label="员工归属" prop="resource">
    ...
    <Modal v-model="editModal" class="iview-right-model" :width="560" :styles="{'margin-right':'0','height':'calc(100% - 50px)','top':'0'}" :transfer="false" :transition-names="['move-right','fade']">
      ...
      <p class="warn-content" style="text-align: center; margin-top: 16px;">更多员工信息，请在员工详情页修改，
        <span style="color:#0bb27a;">去修改</span>
      </p>
      <div slot="footer">
        <el-button @click="editModal = false">取消</el-button>
        <el-button type="primary" :loading="theadLoading" @click="editSbumit">{{editLoading ? '正在保存...' : '保存'}}</el-button>
      </div>
    </Modal>
...
import {
  getSearchTables, // 参数字典下拉框
  phoneCheck
} from '@/api/personnel/advanced_search'
import {
...
import { getAllThead } from '@/api/personnel/roster'
...
export default {
  ...
  data() {
    const validatePhone = (rule, value, callback) => {
      ...
      if (!myreg.test(value)) {
        ...
        phoneCheck(this.editForm.phone, this.userId).then(res => {
    ...
    return {
      ...
      draggData: [], // 顶部拖拽数据
      addDraggData: [], // 待选拖拽
      theadLoading: false, // 保存table表头排序
  ...
  methods: {
    ...
    getTableData() {
      ...
      this.searchform.classifica = this.checked
      this.searchform.depid = this.nowSelectId
      this.searchform.sorting = this.sortId
      getSearchTables(this.searchform).then(res => {
        console.log(res.data)
    ...
    departChange(val) {
      // 部门下拉框改变事件
      this.searchform.depart = val.length === 0 ? null : val[val.length - 1] // 当前选择的部门
    },
    jobChange(val) {
      // 部门下拉框改变事件
      this.searchform.post = val.length === 0 ? null : val[val.length - 1] // 当前选择的部门
    }
  },
  computed: {
    btnitems: function() {
